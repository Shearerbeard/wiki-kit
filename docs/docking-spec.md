...
The orientation index the wiki renders for a consumer lands inside the
dock (as `.wiki/orientation.md`), never committed, regenerated by the
same render pass that produces it today. Harness entry files in the
consumer repo (`CLAUDE.md`, `AGENTS.md`, per-harness equivalents) carry
...
fallback above. Exact per-harness wiring, and the supported-harness set
itself, are K3's deliverables under reserved decision 11.
